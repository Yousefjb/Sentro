    ...
    class Connection
    {
        ...
        public async Task ClearResources()
        {
            ...
            if (CurrentState.HasFlag(State.Caching))
                await CacheManager.Delete(HashCode);
            KvStore.Connections.Remove(HashCode);
            fileLogger.Debug(Tag, "cleared resources");
        }

        private async Task resetTimer(Packet rawPacket)
        {
            await Task.Run(() =>
            {
                timeoutTimer.Stop();
        ...
        private async Task Closed(Packet rawPacket)
        {
            await Task.Run(() =>
            {
                if (rawPacket.Syn)
        ...
        private async Task Establishing(Packet rawPacket)
        {
            await Task.Run(() =>
            {
                if (rawPacket.Ack)
        ...
        private async Task Established(Packet rawPacket)
        {            
            ...
            if (destPort == 80 || destPort == 443)
            {
                ...
                else if (await rawPacket.IsHttpGet())
                {
                    ...
                    if (await CacheManager.IsCached(hashOfLastHttpGet))
                    {
                        ...
                        await SendCacheResponse(response, rawPacket);
                    }
                    else
                    {
            ...
            else
            {
                ...
                else if (await rawPacket.IsHttpResponse())
                {
                    if (hashOfLastHttpGet.Length == 0)
                    {
                        fileLogger.Debug(Tag, "established:response -> post or not get");
                    }
                    else if (await CacheManager.IsCacheable(rawPacket.HttpResponseHeaders))
                    {
        ...
        private async Task Closing(Packet rawPacket)
        {
            await Task.Run(() =>
            {
                if (rawPacket.Ack)
        ...
        private async Task Caching(Packet rawPacket)
        {            
            /*
            ...
            */
            ...
            {
                cachingFileStream = await CacheManager.OpenFileWriteStream(hashOfLastHttpGet);
        ...
        private async Task TryWriteOrderedPackets()
        {            
            while (queuedPackets.ContainsKey(expectedSequence))
            {
                var packet = queuedPackets[expectedSequence];
                queuedPackets.Remove(expectedSequence);
                var packetLength = packet.DataLength;
                await cachingFileStream.WriteAsync(packet.RawPacket, packet.DataStart, packetLength);
                await cachingFileStream.FlushAsync();
        ...
        private async Task SendingCache(Packet rawPacket)
        {
            await Task.Run(() =>
            {
                var destPort = rawPacket.DestPort;
        ...
        private async Task SentCache(Packet rawPacket)
        {            
            var destPort = rawPacket.DestPort;
            if (destPort == 80 || destPort == 443)
            {
        ...
        private async Task SendCacheResponse(CacheResponse response, Packet requestPacket)
        {
            ...
            foreach (var packet in response.NetworkPackets)
            {
                ...
                await SetFakeHeaders(packet, requestPacket, random++, seq, ack, 0);
        ...
        private async Task SetFakeHeaders(Packet response, Packet request, ushort random, uint seq, uint ack, byte fin)
        {
